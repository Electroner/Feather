...
void FeatherGUI::BuildImageDisplayer() {
	...
	//Draw Image
	if (this->workStation.ImagesSize() != 0) {
		if (this->workStation.getImageStrP()->data != NULL) {
			ImGui::Image((void*)(intptr_t)this->workStation.getImageStrP()->texture, ImVec2(this->workStation.getImageStrP()->width * this->zoom, this->workStation.getImageStrP()->height * this->zoom));
		}
	}
	...
	if (this->workStation.ImagesSize() != 0) {
		...
		ImGui::GetWindowDrawList()->AddRect(
			...
			ImVec2(this->workStation.getImageStrP()->width * this->zoom + this->imageShiftX + this->toolsPanelPixels,
				this->workStation.getImageStrP()->height * this->zoom + this->imageShiftY + this->MenuSizePixels),
...
void FeatherGUI::calculateZoom() {
	...
	//Image Sizes
	int image_x = this->workStation.getImageStrP()->width;
	int image_y = this->workStation.getImageStrP()->height;
...
void FeatherGUI::centerImage() {
	...
	this->imageShiftX = static_cast<int>((1.0F - (float)(static_cast<float>(this->toolsPanelPixels + this->propertiesPanelPixels) / static_cast<float>(this->windowWidth))) * io->DisplaySize.x / 2.0F - (this->workStation.getImageStrP()->width * this->zoom / 2.0F));
...
void FeatherGUI::UpdateImage() {
	...
	glBindTexture(GL_TEXTURE_2D, this->workStation.getImageStrP()->texture);

	//Method 1: Update the entire image
	if (this->workStation.getImageStrP()->channels == 4) {
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, this->workStation.getImageStrP()->width, this->workStation.getImageStrP()->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, this->workStation.getImageStrP()->data);
	}
	else 
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, this->workStation.getImageStrP()->width, this->workStation.getImageStrP()->height, 0, GL_RGB, GL_UNSIGNED_BYTE, this->workStation.getImageStrP()->data);
	}
